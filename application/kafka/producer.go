...
func Publish(message string, topic string, producer *ckafka.Producer, deliveryChannel chan ckafka.Event) error {
	producerMessage := &ckafka.Message{
		TopicPartition: ckafka.TopicPartition{Topic: &topic, Partition: ckafka.PartitionAny},
		Value:          []byte(message),
	}

	err := producer.Produce(producerMessage, deliveryChannel)
